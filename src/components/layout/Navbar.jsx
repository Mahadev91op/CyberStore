  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-red-700 to-red-900 text-white/90 text-[11px] font-semibold tracking-wider py-1.5 px-4 text-center md:text-right uppercase">
        ⚡ Free Shipping on Orders Over ₹50,000 | 📞 24/7 Support:
        +91-9876543210
      </div>
      ...
      <header className="bg-[#050505]/90 backdrop-blur-md text-white sticky top-0 z-50 border-b border-white/10 shadow-xl">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between gap-6">
          ...
          <Link href="/" className="group flex items-center gap-2">
            <div className="bg-red-600 text-white h-8 w-8 flex items-center justify-center rounded font-black text-lg group-hover:rotate-6 transition-transform duration-300 shadow-lg shadow-red-900/50">
              C
            </div>
          ...
          <nav className="hidden lg:flex items-center space-x-8 text-sm font-medium text-gray-300">
            <Link
              href="/"
              className="hover:text-white transition relative group py-2"
            >
              Home
            ...
            <div className="relative group py-4 z-50">
              ...
              <div className="absolute top-full left-0 w-64 bg-[#111] border border-gray-800 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform translate-y-4 group-hover:translate-y-0 p-2 flex flex-col gap-1">
                {categories.map((cat) => (
                  ...
                  >
                    {cat.name}
                    <span className="text-red-500 opacity-0 -translate-x-2 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all">
                      ›
                    </span>
                  </Link>
                ))}
            ...
            {user ? (
              <div className="hidden md:flex items-center gap-3 group relative cursor-pointer py-4 z-50">
                ...
                <div className="absolute top-full right-0 w-60 bg-[#111] border border-gray-800 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform translate-y-4 group-hover:translate-y-0 overflow-hidden">
                  ...
                  <Link
                    href="/profile"
                    className="block px-5 py-3 text-sm text-gray-300 hover:bg-white/5 hover:text-white flex items-center gap-3 transition"
                  >
              ...
              <Link
                href="/login"
                className="hidden md:flex items-center gap-2 text-sm font-bold text-gray-300 hover:text-white transition hover:bg-white/5 px-3 py-2 rounded-lg"
              >
                <User size={18} />
                <span>Login</span>
              </Link>
          ...
          <div className="lg:hidden bg-[#050505]/95 backdrop-blur-xl border-t border-gray-800 absolute w-full left-0 shadow-2xl h-[calc(100vh-80px)] z-50 p-6 overflow-y-auto">
            ...
            <ul className="space-y-2">
              <li>
                <Link
                  href="/"
                  className="block p-3 rounded-xl hover:bg-white/5 text-lg font-medium"
              ...
              <li>
                <Link
                  href="/pc-builder"
                  className="block p-3 rounded-xl bg-gradient-to-r from-red-900/20 to-transparent border border-red-900/30 text-red-400 font-bold flex items-center gap-3"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
              ...
              <li className="pt-4 pb-2 text-xs font-bold text-gray-500 uppercase tracking-widest border-t border-gray-800 mt-4">
                Shop by Category
              </li>
              ...
              {user ? (
                <div className="mt-8 pt-6 border-t border-gray-800">
                  ...
                  <button
                    ...
                    className="w-full py-3 rounded-xl bg-red-900/20 text-red-500 border border-red-900/30 font-bold flex items-center justify-center gap-2"
                  >
                ...
                <div className="mt-8 pt-6 border-t border-gray-800 grid grid-cols-2 gap-4">
                  <Link
                    href="/login"
                    className="text-center py-3 rounded-xl bg-white/10 text-white font-bold"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    Login
                  </Link>
                  <Link
                    href="/signup"
                    className="text-center py-3 rounded-xl bg-red-600 text-white font-bold"
                    onClick={() => setIsMobileMenuOpen(false)}
                  >
                    Sign Up
                  </Link>
